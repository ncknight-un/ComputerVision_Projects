...
# Function to use Canny Edge Detection within Hough Transfrom:
#   img: Input Image
#   output: Image with edges detected
def CED_Hough(img: Image, N, sigma, percentageOfNonEdge, max_type, name):
    # Save Filtered Image:
    img = GaussSmoothing(img, N, sigma)
    # Get the Gradient's Magnitude and Direction using Sobel Operator:
    img_mag, img_dir = ImageGradient(img)
